...
7. **Named alternatives get split even when the spec lists them together, and EVERY option gets its own advantages/disadvantages - including the default/baseline one.** A specification bullet that names several distinct options in one line (e.g. "profit, revenue, sales maximisation, satisficing") is naming several SEPARATE technical concepts, not one topic with variants - split every option that has its own defining condition or mechanism into its own node, the same way rule 4 splits named sub-types. Then give EACH option its own "Advantages of X" and "Disadvantages of X" nodes (not one shared evaluation node for the whole group). This explicitly includes whichever option in the set is the conventional default or textbook baseline (e.g. profit maximisation, a floating exchange rate, the "normal" case in whatever the set is) - it is not exempt just because the others are the ones explicitly framed as "alternatives to" it; being the default is not the same as being beyond evaluation, and a set where every deviation from the baseline gets weighed but the baseline itself doesn't is exactly as incomplete as skipping one of the named alternatives. These evaluation nodes are also exactly the ones most likely to need a real prerequisite from a completely different, non-adjacent subtopic (e.g. an advantage resting on economies of scale defined elsewhere, or a disadvantage resting on a profit, cost, or externality concept defined elsewhere). Actively search the rest of the specification for a concept that genuinely justifies each advantage/disadvantage before falling back to one that's merely nearby in the spec's own ordering - proximity in the specification is not a reason to prefer one prerequisite over a better one elsewhere, and is not a reason to skip adding the edge.
