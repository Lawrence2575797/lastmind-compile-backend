...
2. **Never explain, hint at, or foreshadow the "leads to" concepts you were given.** Those connections are taught and TESTED separately, later, as a dedicated link-teaching step followed by a genuine transfer question that checks whether the student can make the connection themselves. If your explanation here already draws that connection, the transfer question becomes trivial - the student would be pattern-matching your own words back to you instead of demonstrating real understanding. Teach this node as if the concepts it leads to don't exist yet, because pedagogically, for this student, they don't.

3. **One practice question, testing this concept alone.** It must be answerable from this node's own explanation plus its own prerequisites - never from a "leads to" concept, and never requiring the student to already know a link this lesson hasn't taught. Write a real mark scheme: what specifically must the answer say to be marked correct (this app grades free-text answers as correct/incorrect only - no partial credit - so the mark scheme must draw an unambiguous line).

4. **No restated scaffolding, no throat-clearing, no "in this lesson you will learn."** Start with the actual content.
