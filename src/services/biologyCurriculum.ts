...
The immediate post-reading practiceQuestion is a recall test of EVERY taught point, not an exam question that samples one detail or demands untaught transfer. Do not reveal answers in the question. Include all required defining conditions, directions, units or mechanisms in its mark scheme. Write exactly THREE questions in all: the practiceQuestion and TWO recallChecks, each covering the SAME complete atomic objective from a different angle, never new facts. Give each a "format", chosen from what the idea is (never for variety): "free_text" (ONLY for a calculation; never for recall, because open recall is asked later by the app itself: a definition is a cloze), "spot_mistake" (at most once across the three questions: a passage of 4-6 short sentences containing every taught point, exactly ONE sentence stating something wrongly; give "segments", "errorIndex" (0-based) and "correction" restating it correctly), "match" (3-5 {left,right} "pairs" in which every taught point is a pair; only when the idea has at least three points), "order" ("items": 3-6 short steps of a process in the CORRECT order that together cover every taught point), or, for a recall check, "cloze" ("text" with ___ per blank and "blanks": [{"answer","alt"}]: the student types the missing terms) or "steps" ("steps": [{"text","keys":[2-4 words, any one of which a correct line must contain]}]: the student types the process in order). The recall checks are delayed retrieval: make at least one of them typed (cloze or steps) unless the idea is a definition, and give any order or match recall check "decoys" (1-3 plausible wrong extra pieces). A spot_mistake sentence must be plausible, never given away by words like never, always or all. Use three different formats across the three questions wherever the idea allows. Whatever the format, the question must still elicit EVERY taught point and list all testedPointIds, and carry a markScheme stating the whole correct answer. Every question must be self-contained: supply the observations, numerical data or description needed to answer; never refer to a missing image, graph, table or apparatus diagram.
...
{"atomicConceptId":"the supplied objective.id","explanation":"1-100 words teaching only the objective","taughtPoints":[{"id":"p1","text":"one essential supporting point"}],"practiceQuestion":{"format":"spot_mistake | match | order | cloze","questionText":"Recall the whole taught idea","markScheme":"Require every taught point; list acceptable equivalents","modality":"writing","answerInputType":"words","testedPointIds":["p1"]},"recallChecks":[{"format":"spot_mistake | match | order | cloze | steps","questionText":"first complete recall prompt","markScheme":"all taught points","testedPointIds":["p1"]},{"format":"spot_mistake | match | order | cloze | steps","questionText":"second complete recall prompt","markScheme":"all taught points","testedPointIds":["p1"]}]}
...
export function validateBiologyEncodingLesson(value: unknown, objective: BiologyObjective): void {
  ...
  content.recallChecks.forEach((question, i) => {
    if (!(isStructured(question) && sanitiseStructured(question))) throw new Error('Biology recall check must be an interactive or typed-recall question');
