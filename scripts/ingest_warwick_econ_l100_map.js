...
const mapPath = path.join(__dirname, '../src/data/warwickEconL100.json');
const map = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
const STAGING = 'Undergraduate Year 1 [warwick-l100 staging]';
const EXPECTED_SPEC = 'BSc Economics L100 Year 1 core (EC104, EC108, EC109, EC124, EC140)';
const clean = value => value.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_');
// Same scheme as spec_lesson_plans' concept_id (see seed_edexcel_econ_lesson_plan.js's conceptId()) so
// specLessonPractice.ts's concept_id lookup lines up with this knowledge map without any extra mapping table.
...
async function main() {
  validate(map);
  if (!map.verified || map.specification !== EXPECTED_SPEC) throw new Error('Map has not passed release review');
  ...
  fs.writeFileSync(path.join(__dirname, 'warwick_econ_build/ingestion-receipt.json'), JSON.stringify(receipt, null, 2));
