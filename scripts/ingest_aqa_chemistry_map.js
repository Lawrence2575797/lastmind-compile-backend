...
const mapPath = path.join(__dirname, '../src/data/aqaChemistryHigher.json');
const map = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
const STAGING = 'GCSE Higher [8462 staging]';
const clean = value => value.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_');
const conceptId = node => `chemistry:${clean(node.subtopic)}:${clean(node.label)}`;
...
async function main() {
  validate(map);
  if (!map.verified || map.rule17 !== true || map.specification !== '8462') throw new Error('Map has not passed release review');
  ...
  const receipt = { publishedAt: new Date().toISOString(), sha256: crypto.createHash('sha256').update(fs.readFileSync(mapPath)).digest('hex'), nodes: published.length, edges: map.edges.length, rule17: true };
  fs.writeFileSync(path.join(__dirname, 'aqa_chemistry_build/ingestion-receipt.json'), JSON.stringify(receipt, null, 2));
