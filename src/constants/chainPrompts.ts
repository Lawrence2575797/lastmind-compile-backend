...
8. GENERALMECHANISM accuracy — for every non-target node, is "generalMechanism" correct? Judge it against the qualification's own ENTRY REQUIREMENT — specifically the qualification BELOW this one (GCSE content for an A-Level target, A-Level content for an Undergraduate Year 1 target), never this same qualification's own earlier topics — not a vague fluency guess: true means the node genuinely belongs on that prior, formally-required course a student cannot reach this qualification without already having passed — e.g. diffusion, osmosis, active transport, Newton's second law, supply and demand — false means it's concept-specific factual, structural, or definitional grounding that exists specifically to understand THIS target, OR a mechanism this qualification's own syllabus introduces properly for the first time (even if it will recur across many of this qualification's own later topics once taught) and wouldn't otherwise meaningfully recur outside this subject. A concrete example: hydrogen bonding and the dipole nature of molecules are commonly marked generalMechanism: true by mistake for an early A-Level Biology/Chemistry target like "the structure and properties of water" — GCSE only gestures at "intermolecular forces" in general, so this is A-Level's own first proper introduction of the mechanism, not entry-level knowledge; it must be false there (correct to false if you find it marked true), even though the same mechanism can legitimately be generalMechanism: true for a LATER A-Level concept once earlier lessons in the course have actually established it. A downstream lesson generator uses this to decide whether a prerequisite gets pre-tested before the target's own derivation (generalMechanism: false) or is instead assumed as a fluent tool the derivation actively recruits in context, without a separate upfront test (generalMechanism: true) — a node wrongly marked true lets genuinely load-bearing, concept-specific grounding slip through untested and asks the student to already know something the course itself is where they first learn it, which is the more damaging direction to get wrong; a node wrongly marked false merely costs an unnecessary check. When you cannot confidently place a mechanism as content from the qualification strictly below this one, correct it TO false rather than leaving it true.
