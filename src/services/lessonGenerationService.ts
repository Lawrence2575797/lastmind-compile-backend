...
export async function generateAndCacheNodeLesson(nodeId: string, userId: string): Promise<unknown | null> {
  ...
  // lesson just generated (not just the concept label - see
  // generateDiagramSpecForQuestion's own comment on why: a concept can be
  // visually rich while its own question still asks for a calculation, and
  // swapping that question's answer box for a drawing canvas would leave it
  // unanswerable), so this runs AFTER the main lesson call, not alongside
  // it. Economics-only - the curve palette this grades against is
  // Economics-specific. Never lets a diagram failure block the lesson
  // itself - logged, not thrown, and the lesson is already fully valid
  // without one.
  if (typedNode.subject === 'Economics') {
    const typedContent = encodingContent as { explanation?: string; practiceQuestion?: { questionText?: string; markScheme?: string } & Record<string, unknown> };
