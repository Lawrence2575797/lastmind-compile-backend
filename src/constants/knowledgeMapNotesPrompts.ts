...
3. "visualType": choose exactly one -
   ...
   - "englishAnchor" — LANGUAGE-LEARNING SUBJECTS ONLY, and only when a genuine, non-forced connection to English exists: a real cognate ("centesimi" sounds like "cent"), a recognisable loanword, or a memorable (not contrived) sound-alike/mnemonic link to an English word or phrase. This is a DIFFERENT mechanism from "example" (an anchor to something the student already knows, in English, not a fresh illustration) and must never be invented where no genuine connection exists — a strained or made-up "sounds like" link is actively worse than no anchor at all, and a FALSE FRIEND (a word that looks/sounds like an English word but means something different) must never be offered as this kind of memory aid, since that would teach the wrong thing. Rare, situational, only when it would genuinely help — not a default for every language concept.
...
6a. If "visualType" is "englishAnchor", also return "englishAnchor": a short (one sentence) plain-text note naming the specific English word/phrase and the genuine connection (e.g. "'Centesimi' shares its root with the English 'cent' - both from the Latin for hundredth.") - never a fabricated etymology, and never phrased as a rule ("always sounds like") when it's really one specific word's own coincidence.
...
{ "heading": string, "paragraphs": string[], "visualType": "comparison" | "workedExample" | "example" | "englishAnchor" | "none", "comparison"?: { "otherLabel": string, "thisPoints": string[], "otherPoints": string[] }, "workedExample"?: { "steps": string[] }, "example"?: string, "englishAnchor"?: string }`;
