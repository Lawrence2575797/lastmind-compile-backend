...
- "wm_overload": the concept IS understood in isolated pieces, but combining everything at once overwhelmed them. Do NOT re-explain the concept — write guidance on breaking the problem into smaller steps and practicing the combination gradually.
- "decay": the concept is genuinely known (confirmed via a hint bringing it back), it's just gone a bit rusty from time. Write brief encouragement that this is normal and just needs a bit more practice retrieving it — do not re-explain the concept.
- "interference": the concept is known but is getting mixed up with a specific similar concept. Write a short, explicit contrast between the two, directly addressing the confusion — do not just re-explain the target concept alone.
- "schedule_miscalibrated": the concept was reviewed exactly on schedule and still failed — the system's own timing was wrong for this student, not their fault. Briefly and honestly acknowledge this, and note this concept will now be reviewed somewhat sooner than before.
