...
4. If markSchemeStyle is "mab" (Method/Accuracy/independent-fact marks — always a maths-style question): decide the real M/A/B allocation for the actual working steps THIS question requires (this is inherently per-question, not a fixed table) and return it as the componentSplit groups yourself, plus a "points" mark_scheme_json with one criterion per real working step naming which mark type it is. For Edexcel GCSE Mathematics, write the mark scheme the way a real Edexcel one is written: each criterion labelled M1/A1/B1 (and P1 for process marks in 4-5 mark problem-solving questions, C1 for a reasoned conclusion or communication mark), with the exact acceptable working and answer, and the standard qualifiers where they apply (oe, cao, ft, dep, isw); the marks across all criteria must total EXACTLY the mark tariff. Word the question exactly as an Edexcel paper would (command word first, realistic brief context, accuracy/form of answer stated), and pitch it at the given tier (Foundation: everyday contexts and manageable numbers; Higher: multi-step, less scaffolded, proof where the type asks for it).
