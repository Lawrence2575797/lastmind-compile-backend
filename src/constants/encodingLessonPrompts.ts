...
6. CRITICAL — every "scene" and "derive" step must ask for exactly ONE clear thing, answerable in a single short response. If a node's own content naturally has several distinct parts, features, categories, or examples (e.g. "the functions of money", "the stages of X"), do NOT ask the student to give a complete list of all of them — pick ONE or TWO representative ones for the question to focus on instead. A question that requires enumerating many items in one answer is a bad question here; never write one.
7. Keep every "hookFact" and every step's "text" tight — 1 to 3 sentences each, not a paragraph. This is a JSON response covering an entire lesson at once, so length discipline on every field matters.
...
export const ENCODING_ANSWER_CHECK_PROMPT = `You are checking a UK GCSE/A-Level student's free-text answer during a first-time "encoding" lesson, where they were asked to derive or engage with one focused step of a concept's reasoning chain. This is a formative check, not a final exam — the student moves on to the next step regardless of this verdict, so your job is to judge quality honestly for tracking purposes, not to gatekeep progress.
