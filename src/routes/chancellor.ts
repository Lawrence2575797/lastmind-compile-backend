...
router.post('/chancellor/interview/assess', actionEndpointLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({
      ...
      headline: str(p?.headline, 140), reaction: str(p?.reaction, 300), coaching: str(p?.coaching, 500), spend,
    });
  } catch (err) {
    const handled = capResponse(res, userId, err);
    if (handled) return handled;
...
router.post('/chancellor/news', actionEndpointLimiter, async (req: Request, res: Response) => {
  ...
  try {
    const { text, spend } = await createAiCall({
      ...
      maxTokens: 900, temperature: 0.7, reason: 'chancellor-news', cacheSystemPrompt: false, clientUsedUsd: Number(b.clientUsedUsd) || undefined,
    });
    const p = parseModelJson<any>(text);
