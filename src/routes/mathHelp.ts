...
router.post('/math-help/threads', costlyEndpointLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const assistantContent = mode === 'advice'
      ? await callClaudeJSON({ model: MODELS.compile, systemPrompt: MATH_HELP_ADVICE_PROMPT, userContent: `Original question: ${questionText}`, userId, meteredReason: 'math-help-start' })
...
router.post('/math-help/threads/:id/messages', costlyEndpointLimiter, async (req: Request, res: Response) => {
  ...
  try {
    ...
    const assistantContent = await callClaudeJSON({ model: MODELS.compile, systemPrompt, userContent: transcript, userId: req.userId as string, meteredReason: 'math-help-message' });
