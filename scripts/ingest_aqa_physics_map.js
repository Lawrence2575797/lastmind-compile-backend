...
const mapPath = path.join(__dirname, '../src/data/aqaPhysicsHigher.json');
const map = JSON.parse(fs.readFileSync(mapPath, 'utf8'));
const STAGING = 'GCSE Higher [8463 staging]';
const clean = value => value.trim().toLowerCase().replace(/[^a-z0-9]+/g, '_');
const conceptId = node => `physics:${clean(node.subtopic)}:${clean(node.label)}`;
...
async function main() {
  validate(map);
  if (!map.verified || map.rule17 !== true || map.specification !== '8463') throw new Error('Map has not passed release review');
  ...
  fs.writeFileSync(path.join(__dirname, 'aqa_physics_build/ingestion-receipt.json'), JSON.stringify(receipt, null, 2));
